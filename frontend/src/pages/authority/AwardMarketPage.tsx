    ...
    <>
      ...
      <StateBlock loading={loading} error={error}>
        <Table headers={["Offre", "Montant", "Score", "Statut", "Action"]}>
          {(data || []).map((offer) => <tr key={offer.id}><td className="px-4 py-3">#{offer.id}</td><td className="px-4 py-3">{formatAmount(offer.montant, offer.devise)}</td><td className="px-4 py-3">{offer.score_total || "-"}</td><td className="px-4 py-3">{offer.statut}</td><td className="px-4 py-3"><Button onClick={() => offerService.award(offer.id)}>Attribuer</Button></td></tr>)}
        </Table>
      </StateBlock>
    </>
  );
