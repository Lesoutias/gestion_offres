...
export default function AwardMarketPage() {
  ...
  return (
    <>
      <PageTitle title="Attribuer le marche" description="Classement par score total (technique + financier)." />
      <StateBlock loading={loading} error={error}>
        <Table headers={["Offre", "Montant", "Score tech.", "Score fin.", "Total", "Statut", "Action"]}>
          {(data || []).map((offer) => (
            <tr key={offer.id}>
